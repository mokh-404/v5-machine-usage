...
collect_load_metrics() {
    ...
    # Strategy A: Windows Native (WSL Fix for accurate Host Uptime)
    if command -v wmic.exe >/dev/null 2>&1; then
        # wmic path Win32_OperatingSystem get LastBootUpTime
        # Output: 20241203120000.000000+120
        local boot_raw=$(wmic.exe os get LastBootUpTime 2>/dev/null | grep -E '^[0-9]+')
        if [ -n "$boot_raw" ]; then
             # Parse YYYYMMDDHHMMSS
             local boot_ts=$(date -d "${boot_raw:0:4}-${boot_raw:4:2}-${boot_raw:6:2} ${boot_raw:8:2}:${boot_raw:10:2}:${boot_raw:12:2}" +%s 2>/dev/null)
             local curr_ts=$(date +%s)
             if [ -n "$boot_ts" ]; then
                 local sec=$((curr_ts - boot_ts))
                 local d=$((sec / 86400))
    ...
    # Strategy B: Standard Linux / Fallback
    if [ -z "$up_str" ]; then
...
# We need to split the original functions or wrap them. 
# Simplest approach: Modify original functions to NOT write to global METRICS if called in async mode?
# Or just let them write to their own isolated subshell METRICS, then print the result.
# The original functions write to METRICS array. In a subshell, this is local.
# So we can just call the original function, then echo the specific METRICS key we want.
